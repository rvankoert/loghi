...
from api_calls import (
    segment_call, extract_baselines_call, cut_from_image_call,
    htr_call, merge_htr_with_pagexml_call, recalculate_reading_order_call,
    split_text_line_into_words_call
)
from utils import (
    file_to_bytes, setup_image_bytes, save_cut_images, xml_to_string
)
...
def process_image(file_path: str, laypa_output_path: str, model_path: str,
                  tooling_output: str, htr_output: str, laypa_address: str,
                  htr_address: str, tooling_address: str) -> None:
    """
    ...
    try:
        ...
        yield image, None, None, None, \
            "Image loaded successfully, starting processing...", None, None

        # Step 1: Perform segmentation
        segmented_image, laypa_result_location, segment_msg = segment_call(
            image, laypa_output_path, model_path, file_name, identifier,
            laypa_address)
        ...
        new_xml, baseline_msg = extract_baselines_call(
            mask_bytes, xml_bytes, image_bytes, identifier, tooling_output,
            tooling_address)
        ...
        cut_success, cut_msg = cut_from_image_call(image_bytes, new_xml_bytes,
                                                   identifier, tooling_output,
                                                   tooling_address)
        ...
        htr_results = htr_call(cut_images_dir, identifier, htr_output,
                               htr_address)
        cut_images = save_cut_images(identifier, tooling_output,
                                     captions=htr_results)
        logging.info('HTR completed.')
        yield image, segmented_image, cut_images, htr_results, \
            "HTR Complete", None, None

        # Step 5: Merge HTR results with Page XML
        merge_success = merge_htr_with_pagexml_call(
            new_xml,
            Path(htr_output) / identifier,
            identifier,
            tooling_address)
        if not merge_success:
            return
        yield image, segmented_image, cut_images, htr_results, \
            "HTR results merged with Page XML.", None, None

        # Step 6: Recalculate reading order
        recalc_success = recalculate_reading_order_call(new_xml, identifier,
                                                        tooling_address)
        if not recalc_success:
            return
        yield image, segmented_image, cut_images, htr_results, \
            "Reading order recalculated.", None, None

        # Step 7: Split text lines into words
        split_success = split_text_line_into_words_call(new_xml, identifier,
                                                        tooling_address)
        final_status = "Splitting lines into words complete." if split_success\
            else "Failed to split text lines into words."
        yield image, segmented_image, cut_images, htr_results, \
            final_status, None, None
        logging.info(final_status)

        # Step 8: Return the final PageXML as a string
        final_page_xml_path = os.path.join(
            tooling_output, identifier, identifier + ".xml")
        xml_string = xml_to_string(final_page_xml_path)
        yield image, segmented_image, cut_images, htr_results, \
            "Workflow complete.", final_page_xml_path, xml_string

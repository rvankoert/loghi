...
GPU=0
datadir=/media/rutger/DIFOR2/train_data_prizepapers/
listdir=/home/rutger/src/loghi-htr/src/
trainlist=/home/rutger/src/loghi-htr/src/training_all_prizepapers_train.txt
validationlist=/home/rutger/src/loghi-htr/src/training_all_prizepapers_val.txt
charlist=/home/rutger/train_data_tmpdir/output_charlist.charlist

epochs=50
height=$HTRLOGHIMODELHEIGHT
#set to 1 for larger datasets, you can increase for smaller datasets
multiply=1
#minimum 2, maximum 5
rnn_layers=3
...
then
        ...
        echo docker run --gpus all --rm -m 32000m --shm-size 10240m -ti -v /tmp:/tmp -v $datadir:$datadir -v $tmpdir:$tmpdir -v $listdir:$listdir docker.htr LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libtcmalloc_minimal.so.4 python3.8 /src/src/main.py --do_train --train_list $trainlist --validation_list $validationlist --learning_rate $learning_rate --channels 4 --batch_size $batch_size --epochs $epochs --do_validate --gpu $GPU --height $height --memory_limit 6000 --use_mask --seed 1  --beam_width 10 --model new10 --rnn_layers $rnn_layers --rnn_units $rnn_units --use_gru --decay_steps 5000 --batch_normalization --multiply $multiply --charlist $charlist --output $listdir --model_name $model_name --output_charlist $tmpdir/output_charlist.charlist --output $tmpdir/output
        docker run --gpus all --rm -m 32000m --shm-size 10240m -ti -v $datadir:$datadir -v $tmpdir:$tmpdir -v $listdir:$listdir docker.htr bash -c "LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libtcmalloc_minimal.so.4 python3.8 /src/src/main.py --do_train --train_list $trainlist \

...
docker run --rm -e HOME=/tmp -u $(id -u "${USER}"):$(id -g "${USER}") $VOLUMES loghi/docker.htr:2.2.21 \
    bash -c "set -e; sed 's/\"None\"/None/g' /src/loghi-htr/utils/convert-v2-to-v3/convert.py >/tmp/convert.py ; chmod +x /tmp/convert.py;  python3 -m pip install --user -r /src/loghi-htr/utils/convert-v2-to-v3/requirements.txt;
